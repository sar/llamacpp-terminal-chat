...
std::string highLightText(std::string text, std::string color,
...
// a dict like python
template <typename keyT> class Dict {
public:
  template <typename valueT> void set(keyT key, valueT value) {
    vals[key] = value;
  }

  template <typename valueT> valueT get(keyT key) {
