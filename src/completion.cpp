...
bool Completion::completionCallback(const std::string &chunk,
  ...
  // Parse the data
  yyjson_doc *doc =
      yyjson_read(completionData.c_str(), strlen(completionData.c_str()), 0);
  ...
  if (using_oai_completion) {
    ...
    if (!yyjson_is_null(finish_reason)) {
      ...
      yyjson_val *predicted_per_second =
          yyjson_obj_get(timings, "predicted_per_second");
...
bool Completion::loadParametersSettings(std::string_view profile_name) {
  ...
  const char *PARAMETER_PROFILE_[] = {"params_profiles", profile_name.data(),
                                      "\0"};
  yyjson_val *parameter_profile = parameters_file.get_value(PARAMETER_PROFILE_);
  if (parameter_profile == NULL)
    return false;
  ...
  while ((key = yyjson_obj_iter_next(&iter))) {
    ...
    if (value_type == "uint") {
      ...
      parameters.set<std::string>(
          param_key, "\"" + std::string(yyjson_get_str(param_value)) + "\"");
...
bool Completion::loadChatTemplates(std::string_view chat_template_name) {
  ...
  if (templates_fp.get_value(main_key) == NULL)
    return false;

  const auto loadSeqToken = [&](const char *key, const char *field_key) {
    const char *keys[] = {"chat_templates", chat_template_name.data(), key,
                          field_key, "\0"};
...
std::string Completion::dumpPayload(yyjson_mut_doc *prompt_json) {
  ...
  yyjson_mut_obj_foreach(prompt_root, idx, max, key, val) {
    ...
    yyjson_mut_obj_add_val(doc, root, keyStr,
                           yyjson_mut_val_mut_copy(doc, val));
  }
  ...
  yyjson_mut_val *hits =
      yyjson_mut_arr_with_str(doc, stop_word_arr, stop_words.size());
  yyjson_mut_obj_add_val(doc, root, "stop", hits);

  // others params
  parameters.forEach([&doc, &root](const std::string &key,
                                   const std::any &value) {
    if (value.type() == typeid(int)) {
      yyjson_mut_obj_add_int(doc, root, key.c_str(), std::any_cast<int>(value));
    } else if (value.type() == typeid(double)) {
      yyjson_mut_obj_add_real(doc, root, key.c_str(),
                              std::any_cast<double>(value));
    } else if (value.type() == typeid(bool)) {
      yyjson_mut_obj_add_bool(doc, root, key.c_str(),
                              std::any_cast<bool>(value));
    } else if (value.type() == typeid(std::string)) {
      yyjson_mut_obj_add_str(doc, root, key.c_str(),
...
Response Completion::requestCompletion(const char *ipaddr, const int16_t port,
...
chat_template_t &Completion::getChatTemplates() { return chat_template; }

void Completion::addStopWord(std::string word) { stop_words.push_back(word); }
