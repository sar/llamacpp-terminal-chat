...
int main(int argc, char *argv[]) {
  ...
  if (chat_template.empty()) {
    ...
    if (!chatContext.loadChatTemplates(chat_template)) {
      Logging::error(
          "Failed to load the custom chat template '%s' ! Using default empty.",
          chat_template.c_str());
      Terminal::pause();
    }
  }
  ...
  if (!chatContext.loadParametersSettings(param_profile)) {
    Logging::error(
        "Failed to load the param profile from '%s' ! Please check it.",
        param_profile.c_str());
    Terminal::pause();
    exit(1);
  }
  ...
  while (true) {
    ...
    if (userInput[0] == '/') {
      ...
      if (cmd == "/help" || cmd == "/h") {
        ...
        std::string narration =
            "*make a short narration in third person describing the current "
            "situation and characters in the chat*";
        ...
        chatContext.setupChatStopWords();
        Logging::success("Params '%s' reloaded!", param_profile.c_str());
        Terminal::pause();
        continue;
        ...
        Logging::success("Template '%s' reloaded", chat_template.c_str());
        Terminal::pause();
        continue;
        ...
        if (arg == "on") {
          ...
          Logging::warn("Invalid argument: '%s'. Expected 'on' or 'off'.",
                        arg.c_str());
          Terminal::pause();
        }
        continue;
        ...
        Logging::success("Chat mode %s",
                         chatContext.isChatMode() ? "ON" : "OFF");
        Terminal::pause();
        continue;
        ...
        std::cout << "[Current user prompt]    : "
                  << (user_prompt.empty() ? "None" : user_prompt) << std::endl;
        std::cout << "[Current params profile] : " << param_profile
                  << std::endl;
        std::cout << "[Current chat template]  : "
                  << (chat_template.empty() ? "OAI mode" : chat_template)
                  << std::endl;
        ...
        std::cout << "[Current params profile] : " << param_profile
                  << std::endl;
        std::cout << std::endl
                  << chatContext.dumpPayload(chatContext.getCurrentPrompt())
                  << "\n\n";
        Terminal::pause();
        continue;
    ...
    Response res = chatContext.requestCompletion(
        ipaddr, port, chatContext.getCurrentPrompt());
    if (res.Status != HTTP_OK) {
      if (res.Status == HTTP_INTERNAL_ERROR)
        Logging::critical("500 Internal server error!");
      if (res.Status == -1)
        Logging::error("Error to connect, please check that server is running "
                       "and try again.");
      if (!res.body.empty())
        Logging::error(
      ...
      if (chatContext.using_oai_completion)
        chatContext.addNewMessage(currentActor, "");
      chatContext.updateMessageContent(chatContext.messagesCount() - 1,
                                       chatContext.completionBus.buffer);
    }
  }
