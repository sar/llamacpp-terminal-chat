...
Response httpRequest::post(
    const char *ipaddr, const int16_t port, const char *endpoint, json payload,
    const std::function<bool(std::string chunck, const CallbackBus *bus)>
        &reader_callback = nullptr,
    const CallbackBus *bus = nullptr) {

  Response response;
  Logging::debug("Connecting to %s to port %d", ipaddr, port);
  ...
  while (streaming and
         (bytesRead = recv(connection, buffer, sizeof(buffer), 0)) > 0) {
    if (bytesRead == 0)
      break;
