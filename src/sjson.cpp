...
yyjson_doc *sjson::open(const char *filename) {
  ...
  if (!current_doc) {
    std::cout << "Problem reading the file \"" << filename << "\": read error ("
              << err.code << "): " << err.msg << " at position:" << err.pos
              << std::endl;
    return NULL;
  }
...
yyjson_val *sjson::get_value(const char *keys[]) {
  ...
  for (int i = 0; *keys[i] != '\0'; keys++) {
    ...
    if (tmp == NULL) {
      std::cerr << "Error getting the value \"" << keys[i] << "\" from \""
                << doc_filename << "\"" << std::endl;
      return NULL;
    }
  }
  return tmp;
...
bool sjson::is_opened() { return current_doc == NULL ? false : true; }
...
yyjson_doc *sjson::get_current_doc() { return current_doc; }

yyjson_val *sjson::get_current_root() { return current_doc_root; }
